...
class TestCFIntegration(BaseTestCase):
    ...
    def test_sp041(self):
        ...
        scored, out_of, messages = self.get_results(check_results)
        assert scored < out_of
        assert ("lat_qc is not a variable in this dataset") in messages
    ...
    def test_fvcom(self):
        ...
        assert ("Unidentifiable feature for variable x") in messages
        assert (
            '§2.6.1 Conventions global attribute does not contain "CF-1.7"'
        ) in messages
    ...
    def test_glcfs(self):
        ...
        assert "source should be defined"
        assert ('units for cl, "fraction" are not recognized by UDUNITS') in messages
