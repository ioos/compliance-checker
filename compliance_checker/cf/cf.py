...
class CFBaseCheck(BaseCheck):
    ...
    def _find_data_vars(self, ds):
        ...
        """
        candidates = {}
        for var_name, variable in ds.variables.iteritems():
    ...
    def check_units(self, ds):
        ...
        for k, v in ds.variables.iteritems():
            ...
            presence = Result(BaseCheck.HIGH, units is not None, (u'§3.1 Variables contain units', k))
            ...
            astring = Result(BaseCheck.HIGH, isinstance(units, basestring), (u'§3.1 units attribute is a string', k))
            ...
            resdeprecated = Result(BaseCheck.LOW, not units in deprecated, (u'§3.1 Variables contain valid units', k))
            ...
            knownu = Result(BaseCheck.HIGH, units_known(units), (u'§3.1 Variables contain valid CF Units', k))
            if not knownu.value:
            ...
            if std_name is not None and std_name in self._std_names:
                ...
                ret_val.append(Result(BaseCheck.HIGH, valid, (u'§3.1 Variables contain valid units for the standard_name', k), msgs))
...
#
    ...
    def check_standard_name(self, ds):
        ...
        for k, v in ds.variables.iteritems():
            ...
            ret_val.append(Result(BaseCheck.HIGH, is_str and in_table, (u'§3.3 Standard Names', k), msgs))
            ...
            if std_name_modifier:
                ...
                ret_val.append(Result(BaseCheck.HIGH, std_name_modifier in allowed, (u'§3.3 Standard Names', k), msgs))
    ...
    def check_flags(self, ds):
        ...
        for k, v in ds.variables.iteritems():
            ...
            result_name = (u'§3.5 Flags and flag attributes', k)
            ...
            if flag_values is not None:
                ...
                fvr = Result(BaseCheck.HIGH, flag_values.dtype == v.dtype, result_name)
                if not fvr.value:
                ...
                fmr = Result(BaseCheck.HIGH, flag_meanings is not None, result_name)
                if not fmr.value:
                ...
                fvsr = Result(BaseCheck.HIGH, len(fvset) == len(flag_values), result_name)
                if not fvsr.value:
            ...
            if flag_meanings is not None:
                fmt = Result(BaseCheck.HIGH, isinstance(flag_meanings, basestring), result_name)
                if not fmt.value:
                ...
                ret_val.append(Result(BaseCheck.HIGH, (ok_count, len(meanings)), result_name, msgs=msgs))

                # now that we've split meanings up, check length vs values/masks

                # 4) number of flag_values must equal number of meanings
                if flag_values is not None:
                    fvfmr = Result(BaseCheck.HIGH, len(flag_values) == len(meanings), result_name)
                    if not fvfmr.value:
                ...
                if flag_masks is not None:
                    fmfmr = Result(BaseCheck.HIGH, len(flag_masks) == len(meanings), result_name)
                    if not fmfmr.value:
            ...
            if flag_masks is not None:
                ...
                ret_val.append(Result(BaseCheck.HIGH, (ok_count, 2), result_name, msgs=msgs))
                ...
                ret_val.append(Result(BaseCheck.HIGH, len(zeros) != 0, result_name, msgs=msgs))
            ...
            if flag_values is not None and flag_masks is not None:
                ...
                allvr = Result(BaseCheck.MEDIUM, all(allv), result_name)
                if not allvr.value:
        ...
        for name, var in ds.variables.iteritems():
            ...
            if getattr(var, u'cell_methods', u'') :
                ...
                for match in re.finditer(psep, method):
                    ...
                    if match.group(u'var') in ds.variables[name].dimensions:
    ...
    def check_compression(self, ds):
        ...
        for name, var in ds.variables.iteritems():
            ...
            if hasattr(var, u'compress'):
                ...
                if all([each in ds.dimensions.keys() for each in getattr(var, u'compress', u'').split(" ")]):
                    valid_form = 1
                else: 
    ...
    def check_feature_type(self, ds):
        ...
        if getattr(ds, u'featureType', u'').lower() in feature_list:
            return Result(BaseCheck.MEDIUM,                            
                        True, u'§9.4 featureType attribute',  
                        reasoning)

        elif getattr(ds, u'featureType', u''):
    ...
    def check_coordinates_and_metadata(self, ds):
        ...
        for each in data_list:
            if getattr(ds.variables[each], u'coordinates', u''):
                result = Result(BaseCheck.MEDIUM,                            \
                            True,                                       \
        ...
        role_list = [getattr(var, u'cf_role', u'').split(u' ') for name,var in ds.variables.iteritems() if hasattr(var, u'cf_role')]
        ...
        if getattr(ds, u'featureType', u'').lower() in single_role and len(np.ravel(role_list)) == 1:
            reasoning = []
            valid = True
        elif getattr(ds, u'featureType', u'').lower() in dual_role and len(np.ravel(role_list)) in [1,2]:
            reasoning = []
            valid = True
        else:
            valid = False
